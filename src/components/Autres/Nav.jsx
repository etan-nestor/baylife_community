            ...
            <div className="flex justify-between items-center px-4 md:px-[8em]">
                <div className="flex items-center gap-4">
                    <div className="md:hidden mr-2">
                        <button className='px-2 py-1 bg-green-500 drop-shadow-md rounded-full' onClick={handleMenuIconClick}>
                ...
                <div className="flex justify-center items-center gap-2 md:gap-5 md:-mr-[7em] -mr-8">
                    ...
                    <div className="flex justify-center items-center gap-2 relative" ref={searchContainerRef}>
                        ...
                        <input className="hidden md:block bg-green-500 border w-[200px] py-2 rounded-lg text-white border-green-400 placeholder-gray-200 shadow-inner drop-shadow-lg outline-none text-xl font-semibold px-7" type="text" ref={searchInputRef} placeholder="Rechercher......" />
                        {showSearch && <Search />}
                    </div>
                ...
                <div className="md:hidden mt-3 ml-1 bg-green-500 dark:bg-gray-800 w-[200px] h-[14rem] rounded-lg">
                    <ul className="flex flex-col items-start justify-center pt-4 gap-4 px-4 py-1">
                        ...
                        <li className="cursor-pointer w-full text-sm dark:text-white  dark:hover:border-green-400 font-semibold text-black px-2 py-1 rounded-md  border border-transparent hover:border-white hover:border-1 transition duration-300 ease-in-out group relative  border-green-400 hover:border-1">
                            <Link to="/membres" className="flex items-center gap-[5px] py-1" onClick={() => setIsMenuOpen(false)}>
                                Membres
                                <span>
                                    <FaCaretDown className="transition-all duration-200 group-hover:rotate-180" />
                                </span>
                            ...
                            <div className="absolute z-[9999] hidden group-hover:block w-[300px] rounded-md bg-green-200 p-2 text-black dark:text-white dark:bg-gray-600 dark:hover:text-green-990">
                                <ul>
                                    {DropDownLinks.map((data) => (
                                        <li key={data.id}>
                                            <Link className="inline-block w-full hover:bg-green-400 rounded-md p-2 dark:hover:bg-black hover:drop-shadow-lg transition duration-300 ease-in-out dark:hover:text-white" to={data.link} onClick={() => setIsMenuOpen(false)}>
                                                {data.name}
                                            </Link>
                                        </li>
                                    ))}
